...
清单中的 `subjects[]` 会记录每个科目的 `id`、题量、相对路径和远端下载地址；`releaseNotes[]` 用于题库热更新弹窗展示本次题库更新内容。题库热更新按科目下载，再在本地聚合给考试逻辑使用。

当远端 manifest 的 `bankTag` 高于本地题库时，应用会提示单独更新题库；桌面端会把下载后的题库按科目写入应用数据目录，网页端也会按科目写入浏览器本地存储，只在运行时为考试逻辑聚合为内存数组。
