...
- 版本号更新为 `0.1.5`。
- 恢复「人工智能导论」题库：从 `v0.1.4` 历史题库中恢复 360 道题，并补充 `subjectId: "ai"`。
...
- 当前总题量 718 道：人工智能导论 360 道，数据结构 358 道。
- 当前数据结构题库包含 358 道题：图片例题、2019-2022 非编程题、自主命题题与专题补充题；2018 文件未包含选择/填空段。
...
- 数据结构题库要求每道题都有题解；人工智能导论按历史题库恢复，暂不包含题解。
...
清单中的 `subjects[]` 会记录每个科目的 `id`、题量、相对路径和远端下载地址。题库热更新按科目下载，再在本地聚合给考试逻辑使用。

当远端 manifest 的 `bankTag` 高于本地题库时，应用会提示单独更新题库；桌面端会把下载后的题库写入应用数据目录，网页端会写入浏览器本地存储。
